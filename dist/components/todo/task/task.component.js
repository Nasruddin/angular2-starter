"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('angular2/core');
var router_1 = require('angular2/router');
var task_service_1 = require('./task.service');
var TaskComponent = (function () {
    function TaskComponent(_taskService) {
        this._taskService = _taskService;
        console.log("[Task component running]");
    }
    TaskComponent.prototype.ngOnInit = function () { this.getTodoById(); };
    TaskComponent.prototype.getTodoById = function () {
        var _this = this;
        this._taskService.retrieveTaskById()
            .subscribe(function (task) { return _this.task = task; }, function (error) { return _this.errorMessage = error; });
    };
    TaskComponent = __decorate([
        core_1.Component({
            selector: "task",
            styles: [require('!raw!autoprefixer?browsers=last 2 versions!sass!./task.scss')],
            template: require('./task.html'),
            directives: [router_1.ROUTER_DIRECTIVES],
            providers: [task_service_1.TaskService]
        }), 
        __metadata('design:paramtypes', [task_service_1.TaskService])
    ], TaskComponent);
    return TaskComponent;
}());
exports.TaskComponent = TaskComponent;
//# sourceMappingURL=task.component.js.map